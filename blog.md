...
- [View project source code](https://github.com/web3sdks-template/burn1155-mint721)
- [Deploy](https://docs.web3sdks.com/deploy)
...
Now, we will use the web3sdks CLI to generate a new hardhat project! So, run this command:

```
npx web3sdks create --contract
```
...
I am going to use the [Next.js Typescript starter template](https://github.com/web3sdks-template/next-typescript-starter) for this guide.

If you are following along with the guide, you can create a project with the template using the [web3sdks CLI](https://github.com/web3sdks/web3/tree/main/packages/cli):

```
npx web3sdks create --next --ts
```

If you already have a Next.js app you can simply follow these steps to get started:

- Install `@web3sdks/react` and `@web3sdks/sdk` and `ethers`.

- Add MetaMask authentication to the site. You can follow this [guide](https://docs.web3sdks.com/guides/add-connectwallet-to-your-website) to add metamask auth.
...
We will now write our smart contracts! Create a new file `BAYClone.sol` in the `contracts` folder. We are going to use the web3sdks contracts extend feature to create a new NFT drop. So, paste the following into it
...
Here, we are importing the `ERC721Drop` contract from the web3sdks contracts package. This contract is a base contract that allows us to create a new NFT drop. We are also passing in the name, symbol, royalty recipient, royalty bps, and primary sale recipient to the constructor.

#### SerumClone.sol

Next, we will create a new contract that will be the collection of the serum. This will be an ERC1155 contract. Create a new file `SerumClone.sol` in the `contracts` folder. We will also use the web3sdks contracts extend feature to create a new ERC1155 collection. So, paste the following into it
...
Here, just like the BAYC Clone, we are extending a base contract from the web3sdks contracts package. This time, we are extending the `ERC1155Base` contract. We are also passing in the name, symbol, royalty recipient, and royalty bps to the constructor.
...
Now that we have written our smart contracts, we will go ahead and deploy our contract using [deploy](https://docs.web3sdks.com/deploy).
...
Once they are uploaded you will be able to see the NFTs! To learn more about batch upload check out [this guide](https://docs.web3sdks.com/guides/how-to-batch-upload).
...
In this guide, we learnt how to use the bases extensions to create a MAYC clone. If you built the Dapp pat yourself on the back and share it with us on the [web3sdks discord](https://discord.gg/KX2tsh9A)! If you want to take a look at the code, check out the [GitHub Repository](https://github.com/web3sdks-template/burn1155-mint721).
